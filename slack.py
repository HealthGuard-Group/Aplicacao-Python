...
if SLACK_BOT_TOKEN:
    ...
    print("[NOTIFICADOR] Cliente Slack inicializado.")
else:
    print("[NOTIFICADOR] AVISO: SLACK_BOT_TOKEN não configurado. Envio desativado.")
...
def formatar_mensagem(idDAC, titulo, subtitulo, nomeIdentificacao, nomeDaMedicao, medidaCapturada):
    blocks = [
        ...
        {"type":"divider"},
        {
            "type":"actions",
            "elements":[
                {"type":"button","text":{"type":"plain_text","text":"Home"},"url": f"{LINK_HOME}"},
                {"type":"button","text":{"type":"plain_text","text":"Ver Detalhes"},"url": f"{LINK_PAINEL}/{idMaquina}"}
            ]
        }
    ]
...
def notificar(idDAC, titulo, subtitulo, nomeIdentificacao, nomeDaMedicao, medidaCapturada, limite, tipo_alerta):
    ...
    canal = procurar_id_canal_slack(idDAC)
    if not canal:
    ...
    blocks_payload = formatar_mensagem(idDAC, titulo, subtitulo, nomeIdentificacao, nomeDaMedicao, medidaCapturada)
